...
def ddim_inference(model, cond, beta, num_steps=None, device='cuda', eta=0.0):
    ...
    shape = (batch_size, 24, cond.shape[2], cond.shape[3])  # 根據 out_channels=24 設置
...
if __name__ == "__main__":
    ...
    # 載入模型
    checkpoint = torch.load("./saved_models/diffusion_model_mul(wind).pth", map_location=device)
    ...
    # 載入資料
    from data_utils_mul import load_and_prepare_data
    import torch
    ...
    gt_day1, gt_day2, gt_day3 = split_days(target_subset)
    ...
    mse_all = mse_fn(generated, target_subset).item()
